...
const getOrdersPage = asyncHandler(async (req, res) => {
  ...
  if (req?.query?.orderDate) {
    ...
    matchCondition.$expr = {
      $in: [{ $year: { date: "$createdAt" } }, orderDateYears],
    };
  }
  ...
  res.render("users/ordersInfo", {
    ...
    orderDate,
    orderStatus,
    bodycss: "/css/myprofile.css",
    bodyjs: "/js/myprofile.js",
  });
});
...
const getOrdersDetails = asyncHandler(async (req, res) => {
  ...
  if (order) {
    let index = order?.orderItems.findIndex(
      (pro) => pro.item._id.toString() === proItem
    );
    order.index = index;
...
const createOrders = asyncHandler(async (req, res) => {
  ...
  let GrandTotal =
    totalPrice + cart?.deliveryCharge + shippingCharge - couponRedeemed;
  let redeemAmount = 0;
  ...
  if (newOrder) {
    if (paymentMethod === "Razorpay") {
      return res.json({ razorpayOrderData });
    } else {
      ...
      for (const item of orderItems) {
        ...
        if (product) {
          ...
          if (sizeInfo) {
            const colorInfo = sizeInfo.find(
              (entry) => entry.color === cartItem.color
            );
            if (colorInfo) {
...
const generateInvoice = asyncHandler(async (req, res) => {
  ...
  const orderData = await Orders.findOne({ orderId, user: req?.user?._id })
    .populate("shippingAddress")
    .populate({
      path: "orderItems.item",
      populate: {
...
const generateSalesReport = asyncHandler(async (req, res) => {
  ...
  const htmlContent = await salesReportGenerator(
    allorders,
    totalGrandTotal,
    startDate,
    endDate
...
const getCheckoutPage = asyncHandler(async (req, res) => {
  ...
  res.render("users/checkout", {
    ...
    wallet,
    Brands,
    bodycss: "/css/checkout.css",
    bodyjs: "/js/checkout.js",
  });
});
...
const orderCancel = asyncHandler(async (req, res) => {
  ...
  if (order) {
    ...
    await order.save();
    res.redirect(
      `/orders/order-details?orderId=${orderId}&proItem=${proItem}&message=${encodeURIComponent(
