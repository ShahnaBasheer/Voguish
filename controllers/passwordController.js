...
const forgotPassword = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    return res
      .status(200)
      .json({ message: "Reset link sent successfully", email });
  } catch (error) {
...
const changePassword = asyncHandler(async (req, res) => {
  try {
    ...
    if (!password || password !== confirmPassword) {
      res.redirect(
        404,
        `/reset-password/${req?.body?.token}?message=${encodeURIComponent(
    ...
    res.redirect(
      302,
      `/login?success=${encodeURIComponent("Password Reset Successfully!")}`
    );
  } catch (error) {
    next(error);
  }
...
const changeOldPassword = asyncHandler(async (req, res) => {
  try {
    ...
    if (newPassword === currentPassword) {
      return res
        .status(400)
        .json({ error: "New password and Current Password are Same!" });
    }
    if (user) {
      ...
      if (!isCurrentPasswordValid) {
        return res
          .status(400)
          .json({ error: "Provided current password is incorrect" });
      }
