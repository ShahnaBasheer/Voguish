...
const addToWallet = asyncHandler(async (req, res) => {
  ...
  if (!wallet && method === "manual_credit") {
    ...
    if (method !== "manual_credit") {
      return res
        .status(400)
        .json({ error: "Invalid method for this operation" });
    }
  ...
  await wallet?.save();
  req.flash("success_msg", `₹${amount} added to wallet successfully!`);
...
const redeemFromWallet = asyncHandler(async (req, res) => {
  ...
  if (wallet?.balance === 0) {
    return res
      ...
      .json({
        error:
          "Insufficient balance in the wallet! Go for another payment method",
      });
  }
